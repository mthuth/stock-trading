...
class PresentationBoundaryTests(unittest.TestCase):
    def test_render_report_context_from_fixture_writes_artifacts(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertIn("Decision Quality Review", dashboard_text)
            self.assertIn("Top 5 Ranked Opportunities Today", dashboard_text)
            self.assertIn("Score Driver Glossary", dashboard_text)
            self.assertIn("Data Maintenance Work Requests", dashboard_text)
            ...
            self.assertLess(dashboard_text.index("Top 5 Ranked Opportunities"), dashboard_text.index("Decision Quality Review"))
            self.assertLess(dashboard_text.index("Decision Quality Review"), dashboard_text.index("Daily Decision Review"))
            ...
            self.assertIn("Decision Quality Review", markdown_text)
            self.assertIn("Top 5 Ranked Opportunities Today", markdown_text)
